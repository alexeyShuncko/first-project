...
const Profile = (props) => {
  ...
  const saveProfile = () => {
    ...
    setOpen(false);
    props
      .getUpdateProfile(data, props.profile.id)
      .then(() => {
  ...
  const followUser = () => {
    props
      .follow(props.profile.user.userId)
      .then(() => {
        props.setOpenSnackBar(true);
      })
      .catch((err) => {
  ...
  const unfollowUser = () => {
    props
      .unfollow(props.profile.user.userId)
      .then(() => {
        props.setOpenSnackBar(true);
      })
      .catch((err) => {
  ...
  return (
    <Container maxWidth={'md'} sx={{ mt: '6rem' }}>
      <Card sx={{ display: { xs: '', md: 'flex' }, p: '1rem', mb: 2 }}>
        <Box
          ...
          <CardMedia
            ...
            component="img"
            src={
              props.profile.user.photos.large !== null
        ...
        <CardContent>
          ...
          <List
            ...
            subheader={
              <ListSubheader sx={{ lineHeight: 2 }}>Contacts:</ListSubheader>
            }
            dense>
          ...
          <CardActions>
            ...
            props.profile.id === props.profile.user.userId ? (
              ...
              props.users.users.find(
                (a) => a.id === props.profile.user.userId
              ) &&
            ...
            {props.users.followingInProgress[0] ===
              props.profile.user.userId && (
              <img
                src={setting}
                alt="settings"
                width={'35px'}
      ...
      <Dialog open={openPhotoEdit} onClose={photoEditClose}>
        ...
        <DialogActions>
          <Button onClick={photoEditClose} variant="contained" color="warning">
            Cancel
          </Button>
          <Button onClick={savePhoto} variant="contained" color="success">
            Save Photo
          </Button>
        </DialogActions>
      </Dialog>
